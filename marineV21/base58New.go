...
func FastBase58Encoding(bin []byte) string {
	//return FastBase58EncodingAlphabet(bin, BTCAlphabet)
	return FastBase58EncodingAlphabet(bin, PaulAlphabet)
}

func Encoding58withType(bin []byte, type_ int8) string {
	//return FastBase58EncodingAlphabet(bin, BTCAlphabet)

	var encoding_rule *Alphabet

	switch type_ {
		
		case 1: //edward25519 
			encoding_rule = PaulAlphabet

		case 2: //ecc(ecdsa)
			encoding_rule = PaulAlphabetECC

		case 3: //ecc(ecdsa)
			encoding_rule = PaulAlphabetRSA
	...
	return FastBase58EncodingAlphabet(bin, encoding_rule)
}
...
func TrivialBase58Encoding(a []byte) string {
	return TrivialBase58EncodingAlphabet(a, PaulAlphabet)
}
...
func Decode_(str string) ([]byte, error) {
...
func FastBase58Decoding(str string) ([]byte, error) {
	return FastBase58DecodingAlphabet(str, PaulAlphabet)
}

func Decoding58withType(str string, type_ int8) ([]byte, error) {
	//return FastBase58EncodingAlphabet(bin, BTCAlphabet)
	...
	switch type_ {
		...
		default:
			return nil, errors.New("Not defined encoding type : " + string(type_) + "? Only supports {edward(1), ecc(2), and RSA(3)")


	}

	return FastBase58DecodingAlphabet(str, encoding_rule)
}
...
func FastBase58DecodingAlphabet(str string, alphabet *Alphabet) ([]byte, error) {
	...
	for _, r := range b58u {
		if r > 127 {
			return nil, fmt.Errorf("FastBase58DecodingAlphabet : High-bit set on invalid digit")
		}
		if alphabet.decode[r] == -1 {
			return nil, fmt.Errorf("FastBase58DecodingAlphabet : Invalid base58 digit (%q)", r)
		}
		...
		if c > 0 {
			return nil, fmt.Errorf("FastBase58DecodingAlphabet : Output number too big (carry to the next int32)")
		}

		if outi[0]&zmask != 0 {
			return nil, fmt.Errorf("FastBase58DecodingAlphabet : Output number too big (last int32 filled too far)")
		}
	}
...
func TrivialBase58Decoding(str string) ([]byte, error) {
	return TrivialBase58DecodingAlphabet(str, PaulAlphabet)
}
